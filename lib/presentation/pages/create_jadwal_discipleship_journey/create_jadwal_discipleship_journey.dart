...
class CreateJadwalDiscipleshipJourney extends StatefulWidget {
  final bool? isEdit;
  final String? id;
  const CreateJadwalDiscipleshipJourney(
      {super.key, this.isEdit = false, this.id});
...
class _CreateJadwalDiscipleshipJourneyState
  ...
  String? _selectedJenisDiscipleshipJourney;
    ...
    return Scaffold(
      appBar: CustomAppBar(
        titleText: widget.isEdit ?? false
            ? "Edit Jadwal Discipleship Journey"
            : "Tambah Jadwal Discipleship Journey",
        showBackIcon: true,
      ),
      body: SafeArea(
        child: Container(
  ...
  Padding _buildImagePreviewCard(FileUploadResponse? item,
      CreateJadwalDiscipleshipJourneyProvider provider, int index) {
    return Padding(
  ...
  void _onGetOneJadwal({String? id}) async {
    ...
    if (response is DataSuccess) {
      setState(() {
        _selectedJenisDiscipleshipJourney =
  ...
  void _onCreateJadwal() async {
    ...
    if (widget.isEdit ?? false) {
      ...
      final response = await createJadwalProvider.createJadwal(
        jenisDisciplehsipJourney: jenisDiscipleshipJourney,
        dateTime: dateTime?.toIso8601String(),
        location: location,
        thumbnail: ImageResponse(
          url: thumbnail.url ?? "",
  ...
  Widget _buildForm(BuildContext context) {
    return Form(
      ...
      child: Column(
        ...
        children: [
          CustomDropdown(
            list: listDiscipleshipJourney,
            value: _selectedJenisDiscipleshipJourney,
            title: "Jenis I Care",
            onChanged: (value) {
              _selectedJenisDiscipleshipJourney = value;
              debugPrint(_selectedJenisDiscipleshipJourney);
            },
          ),
          SizedBox(height: 20),
          CustomTextFormField(
          ...
          SizedBox(height: 20),
          Consumer<CreateJadwalDiscipleshipJourneyProvider>(
