...
class CreateAcara extends StatefulWidget {
  final bool? isEdit;
  final String? id;
  const CreateAcara({super.key, this.isEdit = false, this.id});
...
class _CreateAcaraState extends State<CreateAcara> {
  ...
  Padding _buildImagePreviewCard(
      FileUploadResponse? item, CreateAcaraProvider provider, int index) {
    return Padding(
  ...
  void _onGetOneAcara({String? id}) async {
    final createAcaraProvider = context.read<CreateAcaraProvider>();

    final response = await createAcaraProvider.getOneAcara(id: id);

    if (!mounted) return;

    if (response is DataSuccess) {
      acaraController.text = response.data?.title ?? "";
  ...
  void _onCreateAcara() async {
    ...
    final title = acaraController.text;
    final dateTime = _dates.first;
    final location = locationController.text;
    final thumbnail = createAcaraProvider.getThumbnail();
    final poster = createAcaraProvider.getPoster();
    ...
    if (widget.isEdit ?? false) {
      final response = await createAcaraProvider.updateAcara(
        id: widget.id ?? "",
        title: title,
      ...
      final response = await createAcaraProvider.createAcara(
        title: title,
        dateTime: dateTime?.toIso8601String(),
        location: location,
        thumbnail: ImageResponse(
      ...
      if (response is DataSuccess) {
        ...
        Modal.showSnackBar(
          context,
          text: "Berhasil menambahkan acara",
          snackbarType: SnackbarType.success,
        );
      } else {
        final errorMessage = response.error?.toString() ??
